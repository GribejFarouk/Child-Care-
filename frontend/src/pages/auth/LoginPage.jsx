...
import { Heart, Mail, Lock, ArrowRight, Home, Stethoscope } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Wait a tiny bit for state to propagate, or just navigate based on returned data
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden font-sans">
      ...
      <motion.div
        ...
        style={{ top: '10%', right: '10%', background: 'radial-gradient(circle, rgba(0,122,255,0.06) 0%, transparent 70%)', filter: 'blur(40px)' }}
        animate={{ y: [0, -20, 0], x: [0, 10, 0] }}
        transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        className="absolute w-56 h-56 rounded-full pointer-events-none"
        style={{ bottom: '15%', left: '5%', background: 'radial-gradient(circle, rgba(0,122,255,0.05) 0%, transparent 70%)', filter: 'blur(40px)' }}
      ...
      <motion.div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10" variants={staggerContainer} initial="hidden" animate="show">
        ...
        <motion.p variants={fadeUp} className="mt-2 text-center text-sm text-gray-500">
          Accédez au suivi de santé de vos enfants
        </motion.p>
      </motion.div>
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            <motion.div className="flex items-center justify-between" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
              <div className="flex items-center">
                <input id="remember-me" name="remember-me" type="checkbox" className="h-4 w-4 text-primary-500 focus:ring-primary-500 border-gray-300 rounded transition-colors" />
                <label htmlFor="remember-me" className="ml-2 block text-sm text-gray-600">Se souvenir de moi</label>
              </div>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 }}>
              <button type="submit"
                className="w-full flex justify-center items-center py-3.5 px-4 border border-transparent rounded-xl text-sm font-semibold text-white focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all hover:shadow-lg hover:-translate-y-0.5 group bg-gray-900 hover:bg-gray-800 focus:ring-gray-900">
                Se connecter
          ...
          <div className="mt-6">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-3 bg-white/70 text-gray-400 rounded-full">Nouveau sur ChildCare+ ?</span>
              </div>
            </div>
            <div className="mt-6 space-y-3">
              <Link to="/signup" className="w-full flex justify-center py-3 px-4 border border-gray-200 rounded-xl text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 hover:border-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-all">
                Créer un compte
              </Link>
