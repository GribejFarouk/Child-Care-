...
# --- Database ---
# Uses PostgreSQL when CALENDAR_DB_NAME env var is set, otherwise SQLite for local dev
if os.environ.get('CALENDAR_DB_NAME'):
    DATABASES = {
        'default': {
            'ENGINE': 'django.db.backends.postgresql',
            'NAME': os.environ['CALENDAR_DB_NAME'],
            'USER': os.environ.get('CALENDAR_DB_USER', 'childcare'),
            'PASSWORD': os.environ.get('CALENDAR_DB_PASSWORD', ''),
            'HOST': os.environ.get('CALENDAR_DB_HOST', 'localhost'),
            'PORT': os.environ.get('CALENDAR_DB_PORT', '5432'),
        }
    }
else:
    DATABASES = {
...
REST_FRAMEWORK = {
    'DEFAULT_AUTHENTICATION_CLASSES': (
        'events.authentication.SharedJWTAuthentication',
    ),
