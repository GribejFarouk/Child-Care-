"""
OCR Service Django settings.
PostgreSQL-ready with SQLite fallback for local development.
Pattern identical to other ChildCare+ microservices.
"""
import os
...
# --- Database ---
# Uses PostgreSQL when OCR_DB_NAME env var is set, otherwise SQLite for local dev
if os.environ.get('OCR_DB_NAME'):
    DATABASES = {
        'default': {
            'ENGINE': 'django.db.backends.postgresql',
            'NAME': os.environ['OCR_DB_NAME'],
            'USER': os.environ.get('OCR_DB_USER', 'childcare'),
            'PASSWORD': os.environ.get('OCR_DB_PASSWORD', ''),
            'HOST': os.environ.get('OCR_DB_HOST', 'localhost'),
            'PORT': os.environ.get('OCR_DB_PORT', '5432'),
        }
    }
else:
    DATABASES = {
...
REST_FRAMEWORK = {
    'DEFAULT_AUTHENTICATION_CLASSES': (
        'ocr.authentication.SharedJWTAuthentication',
    ),
